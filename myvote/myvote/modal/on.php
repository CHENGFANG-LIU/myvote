...
$page=$_GET['page'];
$sql="SELECT `topics`.`topic_id`,`total`,`img`,`subject`,`start_time`,`stop_time` FROM `topics` LEFT JOIN (SELECT COUNT(*) AS `total` ,`topic_id` FROM `votes` GROUP BY topic_id)A ON `A`.`topic_id` = `topics`.`topic_id` WHERE `start_time`< '{$today}' && `stop_time`>'{$today}' GROUP BY `topics`.`topic_id` order by `start_time` DESC limit $page ;";
